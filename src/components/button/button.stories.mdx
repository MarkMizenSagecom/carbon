...
Buttons can be rendered with just an icon. To achieve the correct styling for the icon placement, please pass `"center"` to the `iconPosition` prop. This ensures that the icon is centered within the button.
...
Icon only buttons can also display a tooltip message when the icon is hovered over. This can be acheived by passing a string to the `tooltipMessage` prop.

<Preview>
  <Story name="icon only button with tooltip">
    <>
      <Button 
        buttonType="primary" 
        size="small" iconType="bin" 
        iconPosition="center" 
